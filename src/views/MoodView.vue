    ...
    <div class="duo-grid">
      ...
      <div class="duo-card xubao">
        ...
        <div class="sliders">
          <SliderRow
            label="生氣"
            :icon="angerIcon(xubao.anger)"
            v-model="xubao.anger"
            :track-color="angerColor(xubao.anger)"
          />
          <SliderRow
            label="多愛阿仔"
            :icon="loveIcon(xubao.love)"
            v-model="xubao.love"
            track-color="#e07090"
          />
          <SliderRow
            label="整體心情"
            :icon="moodIcon(xubao.mood)"
            v-model="xubao.mood"
            :track-color="moodColor(xubao.mood)"
          />
        </div>
      ...
      <div class="duo-card azai">
        ...
        <div class="sliders">
          <SliderRow
            label="生氣"
            :icon="angerIcon(azai.anger)"
            v-model="azai.anger"
            :track-color="angerColor(azai.anger)"
          />
          <SliderRow
            label="多愛許寶"
            :icon="loveIcon(azai.love)"
            v-model="azai.love"
            track-color="#e07090"
          />
          <SliderRow
            label="整體心情"
            :icon="moodIcon(azai.mood)"
            v-model="azai.mood"
            :track-color="moodColor(azai.mood)"
          />
        </div>
        <div class="duo-footer">
...
interface Entry   { id: number; mood: string; note: string; ts: number }
interface DuoState { anger: number; love: number; mood: number }
interface DuoSaved { anger: number; love: number; mood: number; ts: number }
...
function angerColor(v: number) {
  if (v <= 3) return '#5da038'
  if (v <= 6) return '#e07b20'
  return '#c0392b'
}
...
function moodColor(v: number) {
  if (v <= 3) return '#4a8ab0'
  if (v <= 6) return '#8b7ab0'
  return '#5da038'
}

/* ── Duo state ── */
const XUBAO_KEY = 'duo-xubao'
const AZAI_KEY  = 'duo-azai'

const xubao = ref<DuoState>({ anger: 1, love: 10, mood: 8 })
const azai  = ref<DuoState>({ anger: 1, love: 10, mood: 8 })
const xubaoSaved = ref<string>('')
const azaiSaved  = ref<string>('')
...
function saveXubao() {
  const data: DuoSaved = { ...xubao.value, ts: Date.now() }
  localStorage.setItem(XUBAO_KEY, JSON.stringify(data))
  xubaoSaved.value = formatDate(data.ts)
}
function saveAzai() {
  const data: DuoSaved = { ...azai.value, ts: Date.now() }
  localStorage.setItem(AZAI_KEY, JSON.stringify(data))
  azaiSaved.value = formatDate(data.ts)
}
...
onMounted(() => {
  try { entries.value = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? '[]') } catch { entries.value = [] }
  try {
    const xd: DuoSaved = JSON.parse(localStorage.getItem(XUBAO_KEY) ?? 'null')
    if (xd) { xubao.value = { anger: xd.anger, love: xd.love, mood: xd.mood }; xubaoSaved.value = formatDate(xd.ts) }
  } catch { /* keep default */ }
  try {
    const ad: DuoSaved = JSON.parse(localStorage.getItem(AZAI_KEY) ?? 'null')
...
function saveMood() {
  ...
  entries.value.unshift({ id: Date.now(), mood: selected.value, note: note.value.trim(), ts: Date.now() })
...
.slider-input {
  ...
  background: linear-gradient(
    to right,
    var(--col, #c9a227) 0%,
    var(--col, #c9a227) var(--pct, 0%),
    rgba(139,105,20,.18) var(--pct, 0%),
    rgba(139,105,20,.18) 100%
  );
  outline: none;
  cursor: pointer;
}
...
.duo-save-btn:hover { transform: translate(-1px,-1px); box-shadow: 3px 3px 0 #7a5c10; }
...
.emoji-btn {
  ...
  gap: .25rem;
  padding: .5rem .7rem;
  border: 1.5px solid rgba(139,105,20,.3);
  border-radius: 2px;
...
.emoji-btn.active {
  background: rgba(255,248,210,.9);
  box-shadow: 2px 2px 0 var(--accent, #c9a227);
...
.entry-card {
  ...
  background: linear-gradient(145deg, #fdf8ec 0%, #f5edda 100%);
  border: 1.5px solid rgba(139,105,20,.3);
  border-left: 3px solid var(--entry-color, #c9a227);
  border-radius: 2px;
  box-shadow: 2px 2px 0 rgba(138,110,16,.2);
  transition: transform .2s, box-shadow .2s;
}
